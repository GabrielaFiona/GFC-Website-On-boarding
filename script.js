...
.r-dot {
  ...
  background: var(--bg-dark); border: 2px solid var(--accent-blue);
  border-radius: 50%; box-shadow: 0 0 15px var(--accent-glow); z-index: 2;
}

.r-content h4 {
  color: var(--accent-blue); margin-bottom: 5px; text-transform: uppercase;
...
.progress-header {
  position: sticky; top: 0; background: rgba(5, 5, 8, 0.95); backdrop-filter: blur(12px);
  -webkit-backdrop-filter: blur(12px); z-index: 100; 
  height: 70px;
  border-bottom: 1px solid var(--border-light); display: flex; align-items: center; justify-content: center;
}
.progress-bar { 
  display: flex; width: 90%; max-width: 800px; position: relative; justify-content: space-between; 
  align-items: center; 
}
.progress-line-bg {
  position: absolute; top: 50%; left: 0; width: 100%; height: 1px;
  background: var(--border-light); z-index: 1; border-radius: 2px; transform: translateY(-50%);
}
.progress-line-fill {
  position: absolute; top: 50%; left: 0; width: 0%; height: 1px;
  background: var(--accent-blue); box-shadow: 0 0 10px var(--accent-glow);
  z-index: 2; transition: width 0.6s cubic-bezier(0.22, 1, 0.36, 1); transform: translateY(-50%);
}
.step-indicator {
  position: relative; z-index: 3; background: var(--bg-dark); border: 1px solid var(--border-light);
...
.step-indicator.active {
  border-color: var(--accent-blue); color: var(--accent-blue); box-shadow: 0 0 15px rgba(44, 166, 224, 0.2); transform: scale(1.1);
}
...
.step-label {
  ...
  white-space: nowrap; color: var(--text-muted); opacity: 0.5; transition: opacity 0.3s, color 0.3s;
  line-height: 1;
}
...
input, select, textarea {
  width: 100%; padding: 16px 20px; background: var(--surface-base);
  border: 1px solid var(--border-light); border-radius: 8px;
  font-family: var(--font-body); font-size: 1rem; color: var(--text-main); transition: all 0.3s ease;
}
input::placeholder, textarea::placeholder { color: var(--text-muted); opacity: 0.5; }
input:focus, select:focus, textarea:focus {
  outline: none; border-color: var(--accent-blue); background: var(--surface-hover);
  box-shadow: 0 0 0 4px rgba(44, 166, 224, 0.1);
}
...
.radio-card-label {
  display: block; padding: 12px 15px; background: var(--surface-base);
  border: 1px solid var(--border-light); border-radius: 8px;
...
.radio-card-option input[type="radio"]:checked + .radio-card-label {
  background: rgba(44, 166, 224, 0.15); border-color: var(--accent-blue);
  color: #fff; box-shadow: 0 0 15px rgba(44, 166, 224, 0.2);
}

/* --- UPLOAD DROP ZONE --- */
.upload-drop-zone {
  border: 2px dashed var(--border-light); border-radius: 12px;
  padding: 40px; text-align: center; background: rgba(0,0,0,0.2);
  transition: all 0.3s; cursor: pointer; display: flex; flex-direction: column; align-items: center; justify-content: center;
}
...
/* --- DASHED BUTTON --- */
.btn-dashed {
  width: 100%; padding: 15px; border: 1px dashed var(--border-light);
...
/* --- Buttons (General) --- */
.step-nav-buttons { 
  display: flex; gap: 20px; justify-content: space-between; margin-top: 60px; 
  align-items: center; /* Aligns center items */
}
.btn {
  padding: 18px 40px; border: none; border-radius: 6px;
  font-family: var(--font-body); font-size: 0.85rem; text-transform: uppercase; letter-spacing: 2px; font-weight: 700;
...
/* Updated Download Button Style for Bottom Row */
.btn-download-all {
  display: inline-block; padding: 18px 40px;
  font-size: 0.85rem; background: var(--bg-dark); border: 2px solid var(--accent-blue);
  color: var(--accent-blue); border-radius: 50px; text-transform: uppercase; letter-spacing: 2px; font-weight: 700;
  transition: all 0.3s; cursor: pointer; box-shadow: 0 0 20px rgba(0,0,0,0.5);
  margin: 0; /* Remove auto margins so it fits in flex row */
}
...
.btn-download-mini {
  ...
  padding: 5px 12px; font-size: 0.75rem; border-radius: 4px; cursor: pointer; text-decoration: none; display: inline-block;
}
...
/* Collapsed State: Pushed down so only header shows */
...
/* --- Collapsibles --- */
.collapsible {
  background: var(--surface-base); border: 1px solid var(--border-light); border-radius: 12px;
  overflow: hidden; margin-bottom: 30px; transition: border-color 0.3s ease;
}
.collapsible:hover { border-color: rgba(255,255,255,0.2); }
.collapsible-header {
  width: 100%; border: 0; background: transparent; padding: 25px 30px;
...
.collapsible-chevron {
  font-size: 1.2rem; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center;
  border-radius: 50%; border: 1px solid var(--border-light); color: var(--text-muted); transition: all 0.3s ease;
}
...
.collapsible-body {
  border-top: 1px solid var(--border-light); display: grid; grid-template-rows: 1fr;
  transition: grid-template-rows 0.4s cubic-bezier(0.25, 1, 0.5, 1), border-color 0.4s ease;
}
...
/* --- Packages --- */
.packages-included-inline {
  text-align: center; margin-bottom: 30px; background: var(--surface-base); border-radius: 12px; padding: 20px; border: 1px solid var(--border-light);
}
...
.package-card {
  border: 1px solid var(--border-light); border-radius: 16px; background: var(--surface-base); cursor: pointer; overflow: hidden;
...
.package-expand {
  width: 100%; padding: 10px; background: transparent; border: 1px solid var(--border-light); border-radius: 6px;
  color: var(--text-muted); font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px; margin-top: 10px; transition: all 0.2s;
}
...
.custom-brand-name, .custom-brand-price {
  background: var(--surface-base); border: 1px solid var(--border-light); color: var(--text-main); padding: 10px; border-radius: 6px; width: 100%;
}
...
.suggestion-chip {
  padding: 8px 16px; border: 1px solid var(--border-light); border-radius: 20px; font-size: 0.8rem;
  background: var(--surface-base); color: var(--text-muted); cursor: pointer; transition: all 0.3s;
}
...
.page-tag {
  display: inline-flex; align-items: center; gap: 8px; background: var(--surface-active); border: 1px solid var(--accent-blue);
  color: var(--text-main); padding: 8px 15px; border-radius: 30px; margin: 5px; font-size: 0.9rem;
}
...
.plan-card {
  background: var(--surface-base); border: 1px solid var(--border-light); border-radius: 16px; overflow: hidden; margin-bottom: 20px; transition: all 0.3s ease;
}
/* Draggable Visuals */
.plan-card.dragging { opacity: 0.5; border: 1px dashed var(--accent-blue); }

.plan-card-header {
  padding: 20px 25px; background: rgba(0,0,0,0.2); border-bottom: 1px solid var(--border-light);
  font-family: var(--font-heading); font-size: 1.3rem; display: flex; justify-content: space-between; align-items: center; cursor: pointer;
}
...
.drag-handle {
  cursor: grab;
  font-size: 1.5rem; color: var(--text-muted); padding: 0 10px;
  user-select: none;
}
.drag-handle:active { cursor: grabbing; color: var(--accent-blue); }

.custom-file-upload {
  display: inline-block; padding: 10px 20px; cursor: pointer; background: var(--surface-base); border: 1px dashed var(--border-light); border-radius: 6px;
  color: var(--text-muted); font-size: 0.85rem; transition: all 0.3s; width: 100%; text-align: center;
}
...
.page-file-item {
  font-size: 0.8rem; padding: 6px 10px; border-bottom: 1px solid rgba(255,255,255,0.1); color: var(--text-muted);
...
@media (max-width: 900px) {
  ...
  .roadmap-right { border-left: 2px solid var(--border-light); margin-left: 10px; padding-left: 25px; }
  .step-nav-buttons { flex-direction: column-reverse; gap: 15px; }
  .btn-download-all { width: 100%; text-align: center; }
  .package-grid, .form-grid { grid-template-columns: 1fr; }
  .full-width { grid-column: auto; }
}

/* ======================================================= */
/* --- NEW ADDITIONS FOR STEP 3 VISUAL BUILDER (v2.0) --- */
/* ======================================================= */

/* --- UPDATED VISUAL BUILDER UI (Free Grid + Wireframes) --- */
.builder-layout-container { display: flex; gap: 30px; align-items: flex-start; }
.editor-pane { flex: 1; background: #0f1322; border: 1px solid var(--border-light); border-radius: 12px; padding: 20px; min-height: 600px; position: relative; }
...
/* The Grid Canvas (Updated for Free Movement) */
.grid-canvas { 
  display: grid; 
  grid-template-columns: repeat(12, 1fr); 
  grid-auto-rows: 60px; /* Base row height for snapping */
  gap: 10px; 
  /* Visual guide lines */
  background-image: 
  ...
  border-radius: 8px; 
  position: relative;
  min-height: 600px; /* Ensure space to drop */
  padding-bottom: 100px;
}

/* WIREFRAME VISUAL BLOCKS */
.grid-item { 
  background: #1a1e2e; /* Darker interior for wireframe feel */
  border: 1px solid rgba(255,255,255,0.2); 
  border-radius: 4px; 
  position: absolute; /* Absolute for "Free Floating" */
  transition: box-shadow 0.2s, border-color 0.2s; 
  display: flex; flex-direction: column; 
  box-shadow: 0 4px 10px rgba(0,0,0,0.3);
  z-index: 10;
  /* Visual: Crosshatch pattern for generic blocks */
  background-image: repeating-linear-gradient(45deg, rgba(255,255,255,0.01) 0px, rgba(255,255,255,0.01) 10px, transparent 10px, transparent 20px);
}
.grid-item:hover { border-color: var(--accent-blue); z-index: 20; box-shadow: 0 8px 20px rgba(0,0,0,0.5); }
.grid-item.interacting { opacity: 0.8; z-index: 100; border: 1px dashed var(--accent-blue); cursor: grabbing; }

/* Content Wrapper */
.grid-item-content { width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 10px; position: relative; pointer-events: none; }

/* Visual Icons (Wireframe Style) */
.grid-visual-icon { 
  font-size: 1.8rem; color: var(--accent-blue); margin-bottom: 8px; opacity: 0.8; 
...
.grid-label { 
  ...
  text-align: center; background: rgba(0,0,0,0.6); padding: 2px 8px; border-radius: 4px; 
}

/* Controls */
.grid-remove { position: absolute; top: -8px; right: -8px; color: #fff; background: #ff6b6b; width: 20px; height: 20px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 1rem; opacity: 0; transition: opacity 0.2s; pointer-events: auto; box-shadow: 0 2px 5px rgba(0,0,0,0.3); }
.grid-item:hover .grid-remove { opacity: 1; }

.grid-resize-handle { position: absolute; bottom: 0; right: 0; width: 20px; height: 20px; background: linear-gradient(135deg, transparent 50%, var(--accent-blue) 50%); cursor: se-resize; opacity: 0; pointer-events: auto; }
.grid-item:hover .grid-resize-handle { opacity: 1; }

/* Specific Block Types (Visual Tweaks) */
.block-type-image { background-color: #222; }
.block-type-hero { border-width: 2px; }
.block-type-button { background: var(--accent-blue); color: #fff; border: none; border-radius: 20px; height: 50px !important; min-height: unset; display: flex; align-items: center; justify-content: center; }
.block-type-button .grid-visual-icon { display: none; }
.block-type-button .grid-label { background: transparent; font-weight: 700; font-size: 0.9rem; }

/* --- MOBILE PREVIEW --- */
.preview-pane { width: 300px; flex-shrink: 0; position: sticky; top: 90px; }
.mobile-frame { background: #000; border-radius: 30px; padding: 12px; border: 4px solid #333; box-shadow: 0 20px 40px rgba(0,0,0,0.6); }
.mobile-screen { background: #fff; border-radius: 20px; height: 500px; overflow-y: auto; padding: 15px; display: flex; flex-direction: column; gap: 10px; scrollbar-width: none; border: 1px solid #ddd; }
...
/* Mobile Block Styles (Wireframe) */
.mobile-block { 
  width: 100% !important; background: #f4f4f4; border: 1px solid #ccc; 
  padding: 10px; border-radius: 4px; text-align: center; font-size: 0.7rem; 
  color: #333; min-height: 40px; display: flex; align-items: center; 
  justify-content: flex-start; gap: 10px;
}
.mobile-block-icon { font-size: 1rem; opacity: 0.7; }
.mobile-block-button { background: var(--accent-blue); color: white; border: none; font-weight: bold; justify-content: center; }
